...
export default function MenuManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError('Erro ao salvar item do cardápio. Tente novamente.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const deleteItem = async (id: string) => {
    if (confirm('Tem certeza que deseja excluir este item do cardápio? Esta ação não pode ser desfeita.')) {
      ...
      setError(null);
      try {
        await BetterAuthStorageService.deleteMenuItem(id);
        await loadMenuItems();
        setSelectedItems(selectedItems.filter(itemId => itemId !== id));
      } catch (error) {
        console.error('Error deleting menu item:', error);
        setError('Erro ao excluir item do cardápio. Tente novamente.');
      } finally {
        setIsLoading(false);
      }
    }
